...
- Tool routing and default commands
- Structured output contract and accuracy rules
- Batch and token-efficiency practices
...
python3 scripts/translation_cache.py batch --units .bilingual-paper-digest/translation_units.cached.jsonl --out .bilingual-paper-digest/pending-batch.jsonl --limit 50
```

After translating the batch, merge, validate, and render:

```bash
python3 scripts/translation_cache.py merge --units .bilingual-paper-digest/translation_units.cached.jsonl --batch .bilingual-paper-digest/pending-batch.jsonl
python3 scripts/check_bilingual_quality.py --units .bilingual-paper-digest/translation_units.cached.jsonl
python3 scripts/render_bilingual_markdown.py --units .bilingual-paper-digest/translation_units.cached.jsonl --out output.md
python3 scripts/check_source_alignment.py --units .bilingual-paper-digest/translation_units.cached.jsonl --markdown output.md
```
...
- Export bounded batches and merge them by stable unit ID and source hash; never paste a translated batch over the full units file.
- Render only after the current scope passes deterministic quality checks. Use `render_bilingual_markdown.py --partial` only to emit the fully translated paragraph prefix.
