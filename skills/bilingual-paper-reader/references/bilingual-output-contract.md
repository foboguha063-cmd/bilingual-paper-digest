...
6. Run `scripts/check_digest.py <output.md>` and fix errors.
7. When structured units exist, run `scripts/check_bilingual_quality.py --units <units.jsonl>` before rendering and `scripts/check_source_alignment.py` after rendering. Mark intentional omissions as `skipped`.
8. Manually compare meaning, terminology in context, qualifiers, and ambiguous extraction against the source. Validators catch deterministic mismatches; they cannot certify semantic equivalence.
